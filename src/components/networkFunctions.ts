...
export async function duplicateGraphToDB (
  dataToSave: any, 
  refreshList: Function, 
  setIsDuplicateGraphResponseStatusOk: Function   
  ){    
    saveGraph(dataToSave)
      .then((res) => {
        if (res.data.id) {
          console.log('Duplicated graph saved to the database with this id: ', res.data.id);
          refreshList();
          setIsDuplicateGraphResponseStatusOk(true);
        }        
      })
      .catch(err => {if (err) {console.log(err); setIsDuplicateGraphResponseStatusOk(false)}})
  };